...
4. **Run database migrations** (if you have any custom migration process).
...
A `docker-compose.yml` file is provided for convenience. Make sure the `dating-shared-net`
Docker network exists (it is referenced as an external network). Then start the service:
