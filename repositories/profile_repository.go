...
func (r *ProfileRepository) GetByUserID(userID int) (models.UserProfile, error) {
	...
	err := r.db.QueryRow(`
       SELECT p.id, p.user_id, u.username, p.bio, p.gender, p.date_of_birth,
              ...
              COALESCE(p.civil_status, ''), COALESCE(p.religion, ''), COALESCE(p.religion_detail, ''), COALESCE(p.caste, ''),
              COALESCE(p.height_cm, 0), COALESCE(p.weight_kg, 0), COALESCE(p.dietary_preference, ''), COALESCE(p.smoking, ''), COALESCE(p.alcohol, ''),
              ...
              COALESCE(p.highest_education, ''), COALESCE(p.field_of_study, ''), COALESCE(p.institution, ''), COALESCE(p.employment_status, ''), COALESCE(p.occupation, ''),
